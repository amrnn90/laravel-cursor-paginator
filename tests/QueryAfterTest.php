...
use Amrnn\CursorPaginator\Query\PaginationStrategy\QueryAfter;
...
class QueryAfterTest extends TestCase
{
    ...
    public function it_detects_ordered_by_column()
    {
        ...
        $resultQuery = (new QueryAfter($query, 2))->process(Carbon::create((2008)));
        $this->assertEquals(
            [2009, 2010],
    ...
    public function it_can_handle_multiple_column_ordering()
    {
        ...
        $resultQuery = (new QueryAfter($query, 3))->process(Carbon::create(2002));
        $this->assertEquals(
            [5, 3, 2],
        ...
        $resultQuery = (new QueryAfter($query, 3))->process(Carbon::create(2005));
        $this->assertEquals(
            [6, 1, 5],
            $resultQuery->pluck('id')->all()
        );
